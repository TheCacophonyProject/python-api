...
class Mocked_cacophonyapi(unittest.TestCase):
            ...
            with requests_mock.Mocker() as m:
                m.register_uri(
                    ...
                    "{apiURL}{apiPATH}".format(
                        apiURL=defaults["apiURL"], apiPATH=tc["apiPATH"]
                    ),
    ...
    def test_valid_get_devices(self):
        ...
        with requests_mock.Mocker() as m:
            m.register_uri(
                requests_mock.GET,
                "{apiURL}{apiPath}".format(
                    apiURL=defaults["apiURL"], apiPath=mock_apiPath
                ),
                json=mock_json_result,
                status_code=200,
            )
    ...
    def test_valid_delete_files(self):
        ...
        mock_apiPath = "/api/v1/files/{file_id}".format(file_id=mock_int_file_id)
        mock_json_result = {"rows": {"key1": "value1", "key2": "value2"}}
        mock_qs = {}
        with requests_mock.Mocker() as m:
            m.register_uri(
    ...
    def test_valid_upload_recording(self):
        ...
        for tc in testcases:
            ...
            mock_open[
                os.path.join(os.path.dirname(__file__), tc["filename"])
    ...
    def test_valid_download(self):
        ...
        with requests_mock.Mocker() as m:
            m.register_uri(
                requests_mock.GET,
                "{apiURL}{apiPath}".format(
                    apiURL=defaults["apiURL"], apiPath=mock_apiPathTokenRequest
                ),
        ...
        with requests_mock.Mocker() as m:
            m.register_uri(
                requests_mock.GET,
                "{apiURL}{apiPath}".format(
                    apiURL=defaults["apiURL"], apiPath=mock_apiPathTokenProcess
                ),
                # params={"jwt":token},
    ...
    def test_valid__all_downloads(self):
        ...
        for test in [
            {
                "artifactID": 123490,
                "apipath": lambda int_recording_id: "/api/v1/recordings/{recording_id}".format(
            ...
            {
                "artifactID": 123490,
                "apipath": lambda int_file_id: "/api/v1/files/{file_id}".format(
                    file_id=int_file_id
                ),
    ...
    def _generic_download_test(self, downloadtype):
        ...
        with requests_mock.Mocker() as m:
            ...
            self.assertEqual(m.last_request.qs, {})
            self.assertEqual(
                m.last_request.path, downloadtype["apipath"](downloadtype["artifactID"])
            )
